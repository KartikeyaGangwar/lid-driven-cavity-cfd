...
4. **Extended Unsteady Marching & Bifurcation Cascades:** Through time-accurate physical marching on $513 \times 513$ meshes ($t = 16.0$--$22.0$), we establish the primary supercritical Hopf limit cycle over extended convective time ($St = 0.6664$, recurrence error $\epsilon_{\text{rec}} < 0.08\%$), capture secondary Hopf 2-torus quasi-periodicity ($St = 0.5554$, $10.0$ cycles), and resolve multiple complete periodic cycles across high Reynolds numbers: $Re = 25,000$ ($St = 0.1124$, $2.0$ cycles), $Re = 50,000$ ($St = 0.2399$, $4.0$ cycles), and extreme $Re = 100,000$ ($St = 0.2272$, $3.0$ cycles).
